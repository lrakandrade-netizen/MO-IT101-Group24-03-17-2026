...
1. Managing Employee Records 1.Enter employee information: -Employee Number -Last Name -First Name -SSS Number -PhilHealth Number -TIN -Pag-IBIG Number -Rate Per Day -Days Worked
2 .Click Add Record to save a new employee record. 
3.Select an existing employee record from the table to load its information into the input fields.
4.Click Update Record to save changes to the selected employee.
5.Click Delete Record to remove the selected employee record.

## Computing Payroll

1.Select or enter an employee record. 
2.Click Compute Salary. 
3.The payroll receipt will display: -Gross Salary -SSS Deduction -PhilHealth Deduction -Pag-IBIG Deduction -Total Deductions -Withholding Tax -Net Pay
